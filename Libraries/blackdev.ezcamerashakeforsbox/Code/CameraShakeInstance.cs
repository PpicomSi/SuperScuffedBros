...
namespace EZCameraShake
{
    ...
    public class CameraShakeInstance
    {
        ...
        {
            amt.x = Noise.Perlin(tick, 0) - 0.5f;
            amt.y = Noise.Perlin(0, tick) - 0.5f;
            amt.z = Noise.Perlin(tick, tick) - 0.5f;

            if (fadeInDuration > 0 && sustain)
            {
                if (currentFadeTime < 1)
                    currentFadeTime += Time.Delta / fadeInDuration;
                else if (fadeOutDuration > 0)
                    sustain = false;
            }

            if (!sustain)
                currentFadeTime -= Time.Delta / fadeOutDuration;

            if (sustain)
                tick += Time.Delta * Roughness * roughMod;
            else
                tick += Time.Delta * Roughness * roughMod * currentFadeTime;
